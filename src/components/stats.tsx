		...
		<>
			{stats.map((s) => (
				<Card className={cn("shadow-none dark:ring-0")} key={s.label}>
					<CardHeader>
						<CardTitle className="font-normal text-oklch(0.556 0 0) text-xs dark:text-oklch(0.708 0 0)">
							{s.label}
						</CardTitle>
					</CardHeader>
					<CardContent className="flex flex-col gap-2">
						...
						<div className="flex items-center gap-1 text-xs">
							...
							<span className="text-oklch(0.556 0 0) dark:text-oklch(0.708 0 0)">{s.footnote}</span>
						</div>
					</CardContent>
				</Card>
			))}
