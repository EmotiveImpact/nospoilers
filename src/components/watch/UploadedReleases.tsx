...
function UploadedReleaseScope({search,installationId}:{search:string;installationId?:number|null}) {
  ...
  return <section className="uploaded-releases upload-history" aria-label="Uploaded releases">
    ...
    {!fullDetail?<div className="upload-history-toolbar"><div className="flex items-center gap-2"><span className="text-sm text-mute">Status</span><Select value={statusFilter} onValueChange={value=>{const p=new URLSearchParams(search);p.set('uploadStatus',value);for(const key of ['uploadBefore','upload','uploadFinding','uploadTab'])p.delete(key);navigate(`/watch/releases?${p}`);}}><SelectTrigger aria-label="Status"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="all">All results</SelectItem><SelectItem value="active">Queued or running</SelectItem><SelectItem value="attention">Needs attention</SelectItem><SelectItem value="passed">Policy passed</SelectItem></SelectContent></Select></div><span>Choose a result to preview its decision.</span></div>:<Button variant="outline" onClick={back}>Back to releases</Button>}
