...
function AgentAccessScope({streamId,snapshotId}:{streamId:string;snapshotId:string}){
  ...
  async function save(input:object){const signal=lifetime.current?.signal;if(!signal||signal.aborted||busy)return;setBusy(true);setError('');
    ...
    catch(e){if(!signal.aborted){setView(null);setSecret('');setConfirm(false);setReviewed({});setError(e instanceof Error?e.message:'Agent change failed.');}}finally{if(!signal.aborted)setBusy(false);}
  }
  return <details><summary>Agent tools · explicit access</summary><button type="button" disabled={busy} onClick={()=>{setSecret('');setReload(n=>n+1);}}>Refresh agent activity</button><p>Revoking access stops new tool calls. Existing draft notes remain available for an administrator to review or reject.</p>
