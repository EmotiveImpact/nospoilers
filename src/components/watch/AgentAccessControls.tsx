...
export function AgentAccessControls({streamId,snapshotId}:{streamId:string;snapshotId:string}){
  ...
  useEffect(()=>{const c=new AbortController();void fetch(`/api/release-intelligence/streams/${streamId}/agent-access`,{signal:c.signal,credentials:'same-origin',cache:'no-store'}).then(async r=>{const b=await r.json();if(!r.ok)throw new Error(r.status===403?'Agent access is managed by workspace administrators.':b.error??'Agent access unavailable.');if(!c.signal.aborted){setView(b);setError('');}}).catch(e=>{if(!c.signal.aborted){setView(null);setError(e instanceof Error?e.message:'Agent access unavailable.');}});return()=>c.abort();},[streamId,reload]);
  async function save(input:object){const signal=lifetime.current?.signal;if(!signal||signal.aborted||busy)return;setBusy(true);setError('');
    try{const r=await fetch(`/api/release-intelligence/streams/${streamId}/agent-access`,{method:'POST',signal,credentials:'same-origin',headers:{'content-type':'application/json'},body:JSON.stringify(input)});const b=await r.json();if(!r.ok)throw new Error(b.error??'Agent change failed.');if(!signal.aborted){if(b.token){if(typeof b.token!=='string'||!/^nsa_[a-f0-9-]{36}\.[a-f0-9]{64}$/.test(b.token))throw new Error('Unexpected credential response. Refresh access before trying again.');setSecret(b.token);}else setSecret('');setConfirm(false);setReviewed({});setReload(n=>n+1);}}
    catch(e){if(!signal.aborted)setError(e instanceof Error?e.message:'Agent change failed.');}finally{if(!signal.aborted)setBusy(false);}
  }
  return <details><summary>Agent tools · explicit access</summary><button type="button" disabled={busy} onClick={()=>setReload(n=>n+1)}>Refresh agent activity</button><p>Revoking access stops new tool calls. Existing draft notes remain available for an administrator to review or reject.</p>
