...
export function WorkspaceWebsites({workspaceId,disabledReason,initialUrl='',healthFilter='all',className=''}:{workspaceId:string;disabledReason?:string|null;initialUrl?:string;healthFilter?:string;className?:string}){
 ...
 return <section className={`workspace-websites mt-6 rounded-lg border border-white/10 p-5 ${className}`} aria-labelledby="workspace-websites-heading">
  ...
  <form className="website-connect-form mt-4 flex flex-wrap items-end gap-3" onSubmit={event=>{event.preventDefault();void action(base,{url});}}>
  ...
  {filteredRows?.map(row=><article key={row.id} className="website-source mt-5 border-t border-white/10 pt-4">
   ...
   {!row.verified_at&&!row.disconnected_at&&row.verification_token?<section className="website-verification" aria-labelledby={`website-verification-${row.id}`}>
    <header><h4 id={`website-verification-${row.id}`}>Verify website ownership</h4><p>Add the DNS record below, or publish the same value as an HTTP file. Then verify using your chosen method.</p></header>
    <dl className="website-verification-record">
     <div><dt>DNS record type</dt><dd><code>TXT</code></dd></div>
     <div><dt>Record name</dt><dd><code>_nospoilers.{row.host}</code></dd></div>
     <div className="website-verification-value"><dt>Record value</dt><dd><code>nospoilers-verification={row.verification_token}</code></dd></div>
    </dl>
    <div className="website-verification-method"><Button disabled={busy||!!disabledReason} onClick={()=>void action(`${base}/${row.id}/verify`,{method:'dns'})}>Verify DNS</Button></div>
    <div className="website-verification-alternative"><h5>Alternative: HTTP file</h5><p>Serve the record value above at this URL:</p><code>https://{row.host}/.well-known/nospoilers-verification.txt</code><Button variant="outline" disabled={busy||!!disabledReason} onClick={()=>void action(`${base}/${row.id}/verify`,{method:'http'})}>Verify HTTP file</Button></div>
   </section>:null}
   <div className="website-source-actions mt-3 flex flex-wrap gap-3">
   ...
   {confirming===row.id?<form className="website-disconnect mt-4 rounded-md border border-white/15 p-4" onSubmit={event=>{event.preventDefault();if(confirmation===row.origin_url)void action(`${base}/${row.id}/lifecycle`,{action:'disconnect',confirmation});}}>
    ...
    <div className="mt-3 flex flex-wrap gap-2"><Button type="submit" disabled={busy||!!disabledReason||confirmation!==row.origin_url}>Confirm disconnect</Button><Button type="button" variant="ghost" disabled={busy} onClick={()=>setConfirming(null)}>Cancel</Button></div>
   </form>:null}
