...
function TeamScope({workspaceId}:{workspaceId:string}){
  ...
  useEffect(()=>{const controller=new AbortController();void fetch(`/api/workspaces/${workspaceId}/team`,{signal:controller.signal}).then(async response=>{
    const result=await response.json();if(!response.ok)throw new Error(result.error??'Could not load workspace members.');if(!controller.signal.aborted)setTeam(result);
  }).catch(err=>{if(!controller.signal.aborted){setTeam(null);setError(err.message);}});return()=>controller.abort();},[workspaceId,revision]);
  async function mutate(path:string,method:string,body?:unknown){
    if(busy)return;setBusy(true);setError('');setNotice('');
    try{const response=await fetch(path,{method,headers:{'content-type':'application/json'},body:body===undefined?undefined:JSON.stringify(body)});const result=await response.json();if(!response.ok)throw new Error(result.error??'Could not save access changes.');setNotice('Access changes saved. Existing release evidence is unchanged.');setLogin('');setRevision(v=>v+1);window.dispatchEvent(new Event('nospoilers:workspaces-changed'));}
    catch(err){setError(err instanceof Error?err.message:'Could not save access changes.');}finally{setBusy(false);}
  }
  ...
  return <section className="workspace-management"><header className="watch-release-heading"><div><span className="watch-kicker">Workspace settings</span><h1>Team & access</h1><p>Access to this workspace—not a GitHub login or permission to every workspace in the organisation.</p></div></header>
    ...
    {team?<><div className="workspace-cards" aria-label="Workspace members">{team.members.map(member=><MemberRow key={`${member.user_id}:${member.role}`} member={member} owner={team.role==='owner'} canManage={!!canManage} busy={busy} save={next=>mutate(`/api/workspaces/${workspaceId}/members/${encodeURIComponent(member.user_id)}`,next===null?'DELETE':'PATCH',next===null?undefined:{role:next})}/>)}</div>
      ...
      <section className="mt-8"><h2>Access activity</h2><p className="text-sm text-mute">Latest 100 workspace events. Removing access does not erase historical attribution.</p>{team.events.map(event=><p className="watch-empty" key={event.id}>{event.action.replaceAll('_',' ')} · {event.actor??'Former member'}{event.subject?` → ${event.subject}`:''} · {new Date(event.created_at).toLocaleString()}</p>)}</section></>:null}
  </section>;
}
...
export function WorkspaceInvitationInbox(){
  const [invites,setInvites]=useState<Invitation[]>([]),[error,setError]=useState(''),[busy,setBusy]=useState<string|null>(null),[revision,setRevision]=useState(0);
  ...
  async function accept(id:string){if(busy)return;setBusy(id);setError('');try{const response=await fetch(`/api/workspace-invitations/${id}/accept`,{method:'POST'});const result=await response.json();if(!response.ok)throw new Error(result.error??'Could not accept invitation.');window.dispatchEvent(new Event('nospoilers:workspaces-changed'));navigate(`/watch?workspace=${encodeURIComponent(result.workspaceId)}`);}catch(err){setError(err instanceof Error?err.message:'Could not accept invitation.');}finally{setBusy(null);}}
  return <section aria-label="Workspace invitations">{error?<div className="watch-empty" role="alert">{error}<Button variant="outline" onClick={()=>{setError('');setRevision(v=>v+1);}}>Retry invitations</Button></div>:null}{invites.map(invite=><article key={invite.id} className="watch-empty"><h2>Invitation to {invite.workspace_name}</h2><p>{invite.invited_by} invited you as {invite.role}. Expires {new Date(invite.expires_at).toLocaleDateString()}. Accepting does not connect your GitHub account or start a new trial.</p><Button disabled={!!busy} onClick={()=>void accept(invite.id)}>Accept invitation to {invite.workspace_name}</Button></article>)}</section>;
}
