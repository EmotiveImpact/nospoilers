...
function TeamScope({workspaceId}:{workspaceId:string}){
  ...
  return <section className="workspace-management min-w-0 [overflow-wrap:anywhere]"><header className="watch-release-heading"><div><span className="watch-kicker">Workspace settings</span><h1>Team & access</h1><p>Access to this workspace—not a GitHub login or permission to every workspace in the organisation.</p></div></header>
...
function MemberRow({member,owner,canManage,busy,save}:{member:Team['members'][number];owner:boolean;canManage:boolean;busy:boolean;save:(role:Role|null)=>Promise<void>}){
  ...
  return <article className="min-w-0 [overflow-wrap:anywhere]"><h2>{member.login}</h2><p>{member.role}{member.access_source==='legacy'?' · migrated connection membership':''}</p>{editable?<><div className="flex flex-wrap items-end gap-3"><label className="min-w-0 w-full sm:w-auto sm:flex-1">Role for {member.login}<select value={role} onChange={event=>setRole(event.target.value as Role)}><option value="viewer">Viewer</option><option value="member">Member</option>{owner?<><option value="admin">Admin</option><option value="owner">Owner</option></>:null}</select></label><Button className="h-auto min-h-11 max-w-full whitespace-normal text-left [overflow-wrap:anywhere]" disabled={busy||role===member.role} onClick={()=>void save(role)}>Save role for {member.login}</Button><Button className="h-auto min-h-11 max-w-full whitespace-normal text-left [overflow-wrap:anywhere]" variant="ghost" disabled={busy} onClick={()=>setRemoving(true)}>Remove {member.login}</Button></div>{removing?<div className="watch-empty"><p>Remove {member.login} from this workspace? Their saved evidence remains; a new invitation is required to restore access.</p><Button disabled={busy} onClick={()=>void save(null)}>Confirm removal</Button><Button variant="ghost" onClick={()=>setRemoving(false)}>Cancel</Button></div>:null}</>:null}</article>;
}
