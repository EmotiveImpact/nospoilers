...
function EvidenceSettings({workspaceId,view}:{workspaceId:string;view:'retention'|'audit'}){
 ...
 return <section className="evidence-settings-page min-w-0 space-y-6 [overflow-wrap:anywhere]" aria-label={view==='audit'?'Workspace audit log':'Workspace retention'}>
  ...
  {error?<div role="alert"><p>{error}</p><Button variant="outline" onClick={()=>{setError('');setData(null);setRetry(value=>value+1);}}>Retry</Button>{before?<Button variant="outline" onClick={()=>page('')}>Newest activity</Button>:null}</div>:!data?<WatchSkeleton variant="list" className="mt-4" />:view==='retention'?<>
   ...
   {data.events.length?<div className="mt-6 min-w-0"><EvidenceTable caption="Recorded workspace activity" className="w-full table-fixed text-left text-xs sm:text-sm"><thead className="text-snow"><tr><th scope="col" className="p-2 align-top [overflow-wrap:anywhere] sm:p-3">Action</th><th scope="col" className="p-2 align-top [overflow-wrap:anywhere] sm:p-3">Actor</th><th scope="col" className="p-2 align-top [overflow-wrap:anywhere] sm:p-3">Recorded</th></tr></thead><tbody>{data.events.map(event=><tr key={event.id}><td className="p-2 align-top [overflow-wrap:anywhere] sm:p-3">{event.action.replaceAll('_',' ')}</td><td className="p-2 align-top [overflow-wrap:anywhere] sm:p-3">{event.actor??'Former account'}</td><td className="p-2 align-top [overflow-wrap:anywhere] sm:p-3"><time dateTime={event.created_at}>{new Date(event.created_at).toLocaleString()}</time></td></tr>)}</tbody></EvidenceTable></div>:<p className="watch-empty">No recorded workspace activity on this page.</p>}
