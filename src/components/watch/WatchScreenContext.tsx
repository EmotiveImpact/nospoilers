...
export type WatchScreenContext = {
  ...
  TimelineScreen: ({ previewing, timeline, alerts, alertState, search, onRetryTimeline, onRetryAlerts, }: { previewing: boolean; timeline: { status: "loading"; } | { status: "solo"; } | { status: "ended"; } | { status: "error"; message: string; } | { status: "ready"; entries: { type: "alert" | "alert_event" | "delivery"; at: string; alertId: number | null; repoId?: number | null; title: string | null; kind: string | null; fullName: string | null; action: string | null; actorLogin: string | null; deliveryStatus: "sent" | "failed" | null; inventedIncident: false | null; }[]; days: number; }; alerts: import("@/watch/verdict").DeskAlert[]; alertState: WatchSectionState; search: string; onRetryTimeline: () => void; onRetryAlerts: () => void; }) => import("react").JSX.Element;
