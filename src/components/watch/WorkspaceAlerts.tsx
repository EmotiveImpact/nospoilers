...
function WorkspaceAlertPage({workspaceId,search}:Props){
 ...
 counts={page?.counts} exportLabel="Export retained history"
  ...
  onReviewReleases={()=>{const params=new URLSearchParams({workspace:workspaceId});const install=new URLSearchParams(search).get('install');if(install)params.set('install',install);navigate(`/watch/releases?${params}`);}}
