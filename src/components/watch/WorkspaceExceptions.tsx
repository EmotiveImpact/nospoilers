...
export function WorkspaceExceptions({workspaceId}:{workspaceId:string}){return <ExceptionScope key={workspaceId} workspaceId={workspaceId}/>;}
function ExceptionScope({workspaceId}:{workspaceId:string}){
 ...
 useEffect(()=>{const request=new AbortController();setPage(null);setError('');void fetch(base+(before?`?before=${encodeURIComponent(before)}`:''),{signal:request.signal}).then(async r=>{const data=await r.json();if(!r.ok)throw new Error(data.error??'Exceptions unavailable.');if(!request.signal.aborted)setPage(data);}).catch(e=>{if(!request.signal.aborted)setError(e.message);});return()=>request.abort();},[base,before,revision]);
 useEffect(()=>{
  ...
  async function refresh(){
   try{
    ...
    if(!request.signal.aborted){setEvents(data.events);setDetail(data);}
   }catch(e){if(!request.signal.aborted){setDetail(null);setError(e instanceof Error?e.message:'Exception unavailable.');}}
 ...
 async function decide(action:string){if(!entry||!canDecide||mutation.current)return;const request=new AbortController();mutation.current=request;setBusy(true);setError('');try{const response=await fetch(`${base}/${entry.id}/decision`,{method:'POST',signal:request.signal,headers:{'content-type':'application/json'},body:JSON.stringify({action,note})});const data=await response.json();if(!response.ok)throw new Error(data.error??'Decision not confirmed.');if(!request.signal.aborted){setNotice('Decision recorded. Saved scans and alert resolution are unchanged.');setRevision(v=>v+1);}}catch(e){if(!request.signal.aborted)setError(e instanceof Error?e.message:'Decision failed.');}finally{mutation.current=null;if(!request.signal.aborted)setBusy(false);}}
 return <section className="mt-8 space-y-4" aria-label="Policy exceptions"><header><h2 className="text-xl font-semibold">Policy exceptions</h2><p className="text-sm text-mute">Request from a saved release finding. Approval accepts bounded risk for future checks; it does not fix exposure or change old results.</p></header>
  {error?<p role="alert">{error}</p>:null}{notice?<p role="status">{notice}</p>:null}
  <Button variant="outline" disabled={busy} onClick={()=>setRevision(v=>v+1)}>Refresh exceptions</Button>
  {!page&&!error?<p role="status">Loading exceptions…</p>:null}
