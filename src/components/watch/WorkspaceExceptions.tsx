...
{page?<><ul className="divide-y divide-white/10">{page.exceptions.map(e=><li className="py-3" key={e.id}><button className="min-h-11 max-w-full text-left [overflow-wrap:anywhere]" type="button" aria-pressed={selected===e.id} onClick={()=>choose('exception',e.id)}><strong>{e.rule} · {e.exact_path}</strong><p>{e.effective_status} · expires {new Date(e.expires_at).toLocaleString(undefined,{timeZone:'UTC'})} UTC</p></button></li>)}</ul>{!page.exceptions.length?<p>No exception requests on this page.</p>:null}<nav className="flex flex-wrap gap-3" aria-label="Exception history"><Button variant="outline" disabled={!before||busy} onClick={()=>choose('exceptionBefore',null)}>Newest</Button><Button variant="outline" disabled={!page.nextCursor||busy} onClick={()=>choose('exceptionBefore',page.nextCursor)}>Older</Button></nav></>:null}
  {entry?<article className="watch-card min-w-0 space-y-3 p-4 sm:p-5"><h3>{entry.rule} · {entry.exact_path}</h3><p>{entry.reason}</p><p className="break-all">Scope: {entry.source_origin_id?`Website source ${entry.source_origin_id}`:`Artifact SHA-256 ${entry.artifact_sha256}`}</p><p>Requested by {entry.requested_login} · {independent?'Independent approval required':'Administrator approval required'}</p>
