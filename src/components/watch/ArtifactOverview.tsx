...
function OverviewScope({workspaceId,search}:{workspaceId:string;search:string}) {
 ...
 const alertResponseCount=(data.alertCounts?.open??0)+(data.alertCounts?.waiting??0);
 ...
 return <section className="workspace-overview overview-final" aria-label="Workspace overview">
  <span className="sr-only">{data.workspace.name} · Your releases and saved evidence.</span>
  ...
  {unavailableNotice}
  <section className={`overview-latest is-${priorityAlert?(priorityAlertCritical?'failed':'review'):spotlight?resultTone(spotlight):'neutral'}`} aria-label="Priority release decision" data-scenario={priorityAlertCritical?'exposure':priorityAlert||data.counts.attention>0||(data.alertCounts?.open??0)+(data.alertCounts?.waiting??0)>0?'review':data.counts.active>0?'running':data.counts.total>0?'quiet':'empty'}>
   ...
   <h1>{empty?'Your first release starts here.':priorityAlert?(priorityOperational?'A release check needs your response.':'One release alert needs your response.'):overviewHeading(spotlight)}</h1><h2>{priorityAlert?priorityAlert.title:spotlight?(spotlight.source_kind==='website'?'Website check':'Release artifact'):nextAction.label}{!priorityAlert&&spotlight?<span> · {spotlight.target}</span>:null}</h2>
   <p>{empty?'Upload a build or connect a published release to start collecting evidence.':priorityAlert?'This alert needs a response. Reviewing or resolving it will not rewrite the original scan evidence.':spotlightDescription}</p>
   <Button className="overview-primary" onClick={()=>empty?go('scan',{mode:'package'}):priorityAlert?go('alerts',{alert:String(priorityAlert.id),tab:priorityAlert.status}):spotlight?openEvidence(spotlight.id):navigate(nextAction.href)}>{empty?'Run your first scan':priorityAlert?'Review alert':spotlight?(spotlight.verdict==='Review findings'?'Review finding':'Review evidence'):nextAction.label}<ArrowUpRight aria-hidden/></Button>
   {empty&&(websiteStage||connected)?<button className="overview-link" onClick={()=>go('sources')}>{websiteStage==='verify'?'Verify website ownership':websiteStage==='scan'?'Run your first website check':websiteStage?'Review website setup':'Choose a connected source to check'}<ArrowRight aria-hidden/></button>:null}
   <div className="overview-latest-meta"><span>{data.workspace.name}</span><span>{priorityAlert?dateLabel(priorityAlert.created_at):spotlight?dateLabel(spotlight.created_at):'No saved release yet'}</span><span>{priorityAlert?'Original evidence remains on record':'Production evidence remains a separate lane'}</span></div>
  </section>
  <div className="overview-final-columns"><div className="overview-final-column">
   <section className="overview-needs" aria-label="Saved release reviews"><div className="overview-section-heading"><h2>Release reviews</h2><div className="overview-needs-links">{completedAttention?<button className="overview-link" aria-label={`Completed releases needing review ${completedAttention}`} onClick={()=>go('releases',{uploadStatus:'attention'})}>{completedAttention} {completedAttention===1?'release':'releases'}<ArrowRight aria-hidden/></button>:null}{attemptAttention?<button className="overview-link" aria-label={`Failed attempts needing review ${attemptAttention}`} onClick={()=>go('releases',{releaseView:'attempts',uploadStatus:'attention'})}>{attemptAttention} {attemptAttention===1?'attempt':'attempts'}<ArrowRight aria-hidden/></button>:null}</div></div>
    {actionable.length?<div className="overview-need-list">{actionable.map(row=><button key={row.id} onClick={()=>openEvidence(row.id)}><span><strong>{row.target}</strong><small>{row.source_kind==='website'?'Website evidence':'Release artifact'} · {dateLabel(row.created_at)}</small></span><span className={`overview-status ${resultTone(row)}`}>{resultLabel(row)}</span><ChevronRight aria-hidden/></button>)}</div>:<p className="overview-empty">{data.counts.attention?`${data.counts.attention} saved release ${data.counts.attention===1?'record needs':'records need'} review outside this returned summary.`:'No returned release records need review.'}</p>}
    <p className="overview-micro">Saved scan evidence is reviewed in Releases. Alerts is a separate response queue. <button className="overview-link" aria-label={`Open Alerts response queue, ${alertResponseCount} ${alertResponseCount===1?'alert needs':'alerts need'} response`} onClick={()=>go('alerts')}>{alertResponseCount} {alertResponseCount===1?'alert needs':'alerts need'} response<ArrowRight aria-hidden/></button></p>
   </section>

  <section className="overview-releases" aria-label="Recent release scans"><div className="overview-section-heading"><h2>Latest releases</h2><button className="overview-link" onClick={()=>go('releases')}>All releases<ArrowRight aria-hidden/></button></div>
  ...
  </div><div className="overview-final-column">   <aside className="overview-coverage" aria-label="Coverage status"><div className="overview-section-heading"><h2>Coverage</h2><button className="overview-link" onClick={()=>go('sources',{coverageHealth:'all'})}>Review coverage<ArrowRight aria-hidden/></button></div>
    <div className="overview-coverage-total"><strong>{data.connectedCoverage?.total??0}</strong><span>Connected sources</span></div><dl><div><dt>Connected sources</dt><dd>{data.connectedCoverage?.total??0}</dd></div><div><dt>Recently checked</dt><dd>{data.connectedCoverage?.recent??0}</dd></div><div><dt>Overdue</dt><dd className={(data.connectedCoverage?.delayed??0)>0?'warn':''}>{data.connectedCoverage?.delayed??0}</dd></div><div><dt>Unknown or unavailable</dt><dd className={(data.connectedCoverage?.unknown??0)+(data.connectedCoverage?.unavailable??0)>0?'warn':''}>{(data.connectedCoverage?.unknown??0)+(data.connectedCoverage?.unavailable??0)}</dd></div></dl>
    {data.websiteCoverage?.total?<p>{data.websiteCoverage.total} saved production {data.websiteCoverage.total===1?'website':'websites'} · {data.websiteCoverage.attention} need attention.</p>:<p>Production website evidence is not configured.</p>}
   </aside>
  <section className="overview-activity-section" aria-label="Source activity"><div className="overview-section-heading"><h2>Source activity</h2><button className="overview-link" onClick={()=>go(new URLSearchParams(search).has('install')?'timeline':'releases')}>{new URLSearchParams(search).has('install')?'View timeline':'View history'}<ArrowRight aria-hidden/></button></div>
   ...
   <p className="overview-micro">Activity uses the latest five returned scans, in UTC. Gaps are not proof of inactivity or continuous safety; open the history view for retained evidence.</p>
  </section>
 </div></div></section>;
}
