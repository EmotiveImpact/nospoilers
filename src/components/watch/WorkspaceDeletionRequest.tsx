...
function DeletionForm({organizationId,workspaceId,workspaceName}:{organizationId:string;workspaceId?:string;workspaceName?:string}){
 ...
 return <section className="watch-card mt-6 min-w-0 space-y-4 p-4 text-sm leading-relaxed text-mute [overflow-wrap:anywhere] sm:p-5" aria-label="Data deletion"><h3 className="text-lg font-semibold text-snow">History and account closure</h3><p>Disconnecting GitHub stops monitoring; it does not delete saved evidence or cancel billing.</p><p>Deletion requires explicit authorisation from an organisation owner. Requests are saved for review only: no history is deleted, no account is closed and no subscription is cancelled by this form. Automated deletion is not available yet.</p>
  ...
  <form className="min-w-0 space-y-4" onSubmit={event=>{event.preventDefault();void submit();}}>
   <label className="block min-w-0 space-y-2 text-snow">Request scope <AppSelect label={`Request scope`} className="block min-h-11 w-full min-w-0 rounded border border-white/15 bg-back p-2" value={scope} disabled={busy} onValueChange={(nextValue) => {setScope(nextValue);setConfirmation('');setAcknowledged(false);setRequestId('');setError('');}}>{workspaceId?<option value="workspace_history">History in {workspaceName??'this workspace'}</option>:null}<option value="organization_closure">Close this organisation and delete its history</option></AppSelect></label>
