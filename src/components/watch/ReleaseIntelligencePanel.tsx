...
function IntelligencePanel({ workspaceId, record, supportingReview, decisionControls, comparisonReview }: HistoryProps & { workspaceId: string }) {
  ...
  const errorPanel=useRef<HTMLDivElement|null>(null),lastFocused=useRef<HTMLElement|null>(null);
  useEffect(()=>{
    // Restore a useful keyboard destination only when the failed action removed its control.
    // Do not steal focus if the customer has already moved elsewhere on the page.
    if(error&&lastFocused.current&&!lastFocused.current.isConnected&&document.activeElement===document.body)errorPanel.current?.focus();
  },[error]);
  ...
  useEffect(() => {
    const controller = new AbortController(); setView(current=>current?.stream.id===streamId?current:null);
    if (!streamId) {setRefreshing(false);return () => controller.abort();}
    setRefreshing(true);
    ...
    void read<View>(`${API}/streams/${streamId}?${params}`, controller.signal).then(body => {
      ...
      setView(body);
    }).catch(e => { if (!controller.signal.aborted) {setView(null);setError(e instanceof Error ? e.message : 'Stream unavailable.');} }).finally(()=>{if(!controller.signal.aborted)setRefreshing(false);});
    return () => controller.abort();
  ...
  async function save(action: 'create' | 'capture' | 'baseline' | 'exclusions', input: object, message: string) {
    const signal = lifetime.current?.signal; if (!signal || busy || refreshing) return;
  ...
  return <section ref={root} className="ns-intelligence" aria-labelledby={`history-${record.kind}-${record.id}`} aria-busy={busy || refreshing} onFocusCapture={event=>{if(event.target instanceof HTMLElement)lastFocused.current=event.target;}}>
    ...
    {!list ? (!error?<WatchSkeleton variant="list" label="Reading release streams"/>:null) : <>
      {!list.streams.length ? null : <div className="ns-intelligence__actions"><label>Release stream<AppSelect label={`Release stream`} disabled={busy || refreshing} value={streamId} onValueChange={(nextValue) => { setStreamId(nextValue); setSnapshotId(''); setBefore(''); setError(''); setNotice(''); }}><option value="">Choose a stream</option>{list.streams.map(s => <option key={s.id} value={s.id}>{s.name} · {s.artifact_role} · {s.channel}</option>)}</AppSelect></label>{streamId && view?.canWrite && !linked ? <button type="button" disabled={busy || refreshing} onClick={() => void save('capture', { record }, 'This signed release was added to the selected history.')}>Add this release to history</button> : null}</div>}
