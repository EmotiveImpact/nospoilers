...
function GithubWorkspaceConnectScope({workspaceId,disabledReason}:{workspaceId:string;disabledReason?:string|null}){
 ...
 async function start(){
  ...
  try{
   ...
   const body=await response.json() as {url?:string;error?:string};if(!active.current)return;if(!response.ok||!body.url)throw new Error(body.error??'Could not start the connection.');
