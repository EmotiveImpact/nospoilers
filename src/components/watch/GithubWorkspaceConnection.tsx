...
function GithubWorkspaceConnectScope({workspaceId,disabledReason,compact=false}:ConnectProps){
 ...
 async function start(newInstallation=false){
  if(submitting.current||disabledReason)return;submitting.current=true;setBusy(true);setError(null);setReauth(false);
  try{
   ...
   const body=await response.json() as {url?:string;status?:string;installations?:ExistingInstallation[];error?:string;code?:string};if(!active.current)return;
   if(!response.ok){setReauth(body.code==='github_reauth_required');throw new Error(body.error??'Could not start the connection.');}
 ...
 const recovery=reauth?<div className="mt-3"><Button as="a" href="/api/auth/github">Sign in to GitHub again</Button><p className="mt-2 text-sm text-mute">Use the same GitHub account to restore its authorisation. Then return to this workspace and retry Connect source.</p></div>:null;
