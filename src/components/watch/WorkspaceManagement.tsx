...
export function WorkspaceManagement(){
  const [rows,setRows]=useState<ProductWorkspace[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState<string|null>(null);
  ...
  useEffect(()=>{const controller=new AbortController();void fetch('/api/workspaces',{signal:controller.signal}).then(async response=>{
    if(!response.ok)throw new Error('Could not load workspaces.');const body=await response.json() as {workspaces:ProductWorkspace[];organizations?:ManagedOrganization[]};if(!controller.signal.aborted){setRows(body.workspaces);setOrganizations(body.organizations??[]);setLoading(false);}
