...
function NotificationScope({workspaceId}:{workspaceId:string}){
 ...
 return <section className="space-y-6" aria-label="Workspace notifications">
  ...
  {page?.canManage?<form className="watch-card p-4 sm:p-5 flex flex-col items-stretch gap-4 sm:flex-row sm:flex-wrap sm:items-end" onSubmit={e=>{e.preventDefault();void act('save');}}>
   <label className="w-full sm:w-auto">Destination type<select disabled={busy} className="block w-full rounded border border-white/15 bg-back p-2" value={kind} onChange={e=>{setKind(e.target.value as 'email'|'slack');setValue('');}}><option value="email">Email</option><option value="slack">Slack</option></select></label>
