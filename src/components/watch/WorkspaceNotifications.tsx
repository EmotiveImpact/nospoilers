...
import './design/notification-journey.css';
...
function NotificationScope({workspaceId,connectionSettings}:{workspaceId:string;connectionSettings?:ReactNode}){
 ...
 const setSelected=(selected:Destination|null)=>setDisconnect({selected,confirm:''});
 const setConfirm=(confirm:string)=>setDisconnect(current=>({...current,confirm}));
  ...
  async function refresh(){
   try{
    ...
    if(!request.signal.aborted){setPage(data);setLoadError('');
     ...
     if(!data.canManage){setValue('');setComposing(false);testKeys.current.clear();}
    }
   }catch(e){if(!request.signal.aborted){setPage(null);setDisconnect({selected:null,confirm:''});setLoadError(e instanceof Error?e.message:'Could not load notifications.');}}
 ...
 async function act(action:'save'|'test'|'disconnect',destination?:Destination){
  ...
  if(destination&&!page.destinations.some(d=>d.independent&&d.id===destination.id&&d.host===destination.host))return;
 ...
 const destinations=<>
  ...
  {page?<><div className="notification-table-wrap"><table className="notification-journey-table"><caption className="sr-only">Connected destinations</caption><thead><tr><th scope="col">Destination</th><th scope="col">Type</th><th scope="col">Status</th><th scope="col"><span className="sr-only">Actions</span></th></tr></thead><tbody>{page.destinations.filter(d=>d.independent).map(d=><tr key={d.id}><td><div className="notification-entity"><span className="notification-symbol" aria-hidden><Mail size={18}/></span><div><strong>{d.host}</strong><small>Last delivery: {d.last_delivery_status??'not sent'}</small></div></div></td><td>{d.kind==='email'?'Email':'Slack'}</td><td><span className={`notification-delivery-state is-${d.test_status??'untested'}`}>{notificationTestLabel(d.test_status)}</span></td><td>{page.canManage?<div className="notification-actions"><Button variant="outline" aria-label={`Send test to ${d.host}`} disabled={busy||!page.providers[d.kind as 'email'|'slack']||d.test_status==='queued'||d.test_status==='running'} onClick={()=>void act('test',d)}>Send test</Button><Button id={disconnectTriggerId(d.id)} variant="ghost" aria-label={`Disconnect ${d.host}`} disabled={busy} onClick={()=>{setSelected(d);setConfirm('');}}>Disconnect</Button></div>:null}</td></tr>)}</tbody></table></div>
   ...
   <section className="notification-channel-section" aria-label="Available channels"><h2>Available channels</h2><div className="notification-channel-row"><div><h3>Email</h3><p>{page.providers.email?'Sending is configured. Save a destination, then test delivery.':'Provider sending is not configured on this host.'}</p></div><span className={page.providers.email?'is-available':''}>{page.providers.email?'Available':'Unavailable'}</span></div><div className="notification-channel-row"><div><h3>Slack</h3><p>{page.providers.slack?'Requires Team or an active trial. Test after connecting.':'Provider sending is not configured on this host.'}</p></div><span className={page.providers.slack?'is-available':''}>{page.providers.slack?'Available':'Unavailable'}</span></div><div className="notification-channel-row"><div><h3>Delivery history</h3><p>Check the recorded outcome of each message.</p></div><Button variant="ghost" onClick={()=>setTab('history')}>Open history</Button></div></section>
   <aside className="notification-scope-note" aria-label="Which alerts are sent"><p>These destinations receive independent website alerts. Uploaded packages produce scan evidence, not ongoing alerts. Slack requires Team or an active trial.</p>{page.destinations.some(d=>!d.independent)?<p>GitHub connection destinations are managed in the GitHub tab. Their delivery history is included here.</p>:null}</aside>
  </>:null}
 ...
 return <section className="notification-page notification-settings" aria-label="Workspace notifications">
  ...
  <SettingsTabs value={tab} onValueChange={setTab} label="Notification settings" tabs={[{id:'destinations',label:'Destinations',content:destinations},{id:'history',label:'Delivery history',content:history},...(connectionSettings?[{id:'github',label:'GitHub',content:connectionSettings}]:[])]}/>
 </section>;
}
