...
function NotificationScope({workspaceId}:{workspaceId:string}){
 ...
 return <section className="space-y-6" aria-label="Workspace notifications">
  ...
  {page?<><ul className="divide-y divide-white/10">{page.destinations.filter(d=>d.independent).map(d=><li className="flex flex-wrap items-center justify-between gap-4 py-4" key={d.id}><div><strong>{d.kind==='email'?'Email':'Slack'} · {d.host}</strong><p className="text-sm text-mute">Last delivery: {d.last_delivery_status??'not sent'} · Latest test: {notificationTestLabel(d.test_status)}</p></div>{page.canManage?<div className="flex gap-3"><Button variant="outline" disabled={busy||!page.providers[d.kind as 'email'|'slack']||d.test_status==='queued'||d.test_status==='running'} onClick={()=>void act('test',d)}>Send test to {d.host}</Button><Button variant="ghost" disabled={busy} onClick={()=>{setSelected(d);setConfirm('');}}>Disconnect {d.host}</Button></div>:null}</li>)}</ul>
