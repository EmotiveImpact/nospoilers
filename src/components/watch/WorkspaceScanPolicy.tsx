...
function ScopedPolicy({workspaceId,connectionPanels}:{workspaceId:string;connectionPanels?:ConnectionPanels}){
 ...
 const rules=<section className="policy-rules" aria-label="Scan rules">
  ...
  {policy&&<>{!policy.supported?<p className="policy-notice">Independent scan policy is unavailable for this workspace.</p>:<form className="policy-rules-form policy-journey-form" onSubmit={event=>{event.preventDefault();void save();}}>
 ...
 return <section className="scan-policy-page" aria-label="Workspace scan policy"><WatchPageHeader title="Define what needs review." lede="Rules apply to scans in this workspace. Changes are explicit and auditable."/><SettingsTabs label="Policy settings" tabs={tabs} value={activeTab} onValueChange={value=>setTab({search,value})}/></section>;
}
