...
function ScopedPolicy({workspaceId,connectionPanels}:{workspaceId:string;connectionPanels?:ConnectionPanels}){
 ...
 const rules=<section className="policy-rules" aria-label="Scan rules">
  ...
  {policy.events.length>0&&<details className="policy-scope-note policy-history"><summary>Policy history</summary><ul>{policy.events.map(event=><li key={event.revision}>Revision {event.revision} · {event.strict?'Strict':'Critical findings'} · {new Date(event.created_at).toLocaleString()} · User {event.actor_user_id}</li>)}</ul></details>}
  </>}
 </section>;
