...
type Props={alertId:number;workspaceId?:string;value:string;onChange:(id:string)=>void;inputRef:RefObject<HTMLButtonElement|null>};
...
function MemberOptions({alertId,workspaceId,value,onChange,inputRef}:Props){
 ...
 return <div className="mt-5"><label className="block"><span className="text-xs text-mute">Workspace member</span><AppSelect label={`Workspace member`} ref={inputRef} required value={value===CLEAR_ALERT_ASSIGNMENT||members?.some(member=>member.id===value)?value:''} disabled={!members||error} onValueChange={(nextValue) => onChange(nextValue)} className="mt-2 h-12 w-full rounded-md border border-white/15 bg-panel px-3 text-sm text-snow"><option value="">{members?'Choose a teammate':'Loading members…'}</option><option value={CLEAR_ALERT_ASSIGNMENT}>Unassigned — clear ownership</option>{members?.map(member=><option key={member.id} value={member.id}>{member.login}</option>)}</AppSelect></label>{error?<div><p role="alert">Workspace members could not be loaded.</p><Button type="button" variant="outline" onClick={()=>{setError(false);setRetry(value=>value+1);}}>Retry members</Button></div>:null}</div>;
}
