"use client";
// Adapted from beui.dev/components/motion/select (MIT; docs/third-party/beui-LICENSE.txt).
// NoSpoilers adds accessible trigger naming and keyboard/focus navigation.
...
export function Select({
  ...
  useEffect(() => {
    if (!open) return;
    const onKey = (e: KeyboardEvent) => { if (e.key === "Escape") { setOpen(false); document.getElementById(`${baseId}-trigger`)?.focus(); } };
  ...
  return (
    <SelectContext.Provider value={ctx}>
      <div ref={rootRef} className={cn("relative", className)} onKeyDown={event => {
        ...
        requestAnimationFrame(() => {
          ...
          const next = event.key === 'Home' ? 0 : event.key === 'End' ? options.length - 1 : !wasOpen || index < 0 ? Math.max(0,selected) : (index + (event.key === 'ArrowDown' ? 1 : options.length - 1)) % options.length;
          options[next]?.focus();
        });
      }}>
        {children}
