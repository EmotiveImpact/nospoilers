...
export async function readBoundedBody(body: ReadableStream<Uint8Array> | null, maxBytes:number, declared?:string|null,timeoutMs=30_000,signal?:AbortSignal):Promise<Buffer> {
  ...
  const interrupted=new Promise<never>((_,reject)=>{
    onAbort=()=>{reject(Object.assign(new Error('Body read cancelled.'),{status:499}));void reader.cancel().catch(()=>undefined);};
  ...
  try {
    while(true){const result=await Promise.race([reader.read(),deadline,interrupted]);signal?.throwIfAborted();if(result.done)break;size+=result.value.byteLength;
