...
function priceIdFromObject(object: Record<string, unknown>): string | null {
  ...
  return asString(price.id) ?? asString(object.price) ?? asId(asRecord(object.lines).data);
}
...
function eventPatch(event: StripeEvent, prices: StripePriceMap): StripeBillingPatch | null {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    const metaPlan = parseStripePlan(metadataOf(object).plan);
    ...
    return {
      ...
      plan: metaPlan ?? planFromStripePrice(prices, priceId),
      clearPlan: false,
    };
  }
  if (
    ...
    const plan = parseStripePlan(metadataOf(object).plan) ?? planFromStripePrice(prices, priceId);
    ...
    return {
      ...
      clearPlan: Boolean(status) && !covers && status !== "incomplete",
    };
  }
  ...
  if (event.type === "invoice.paid") {
    ...
    return {
      ...
      priceId,
      periodEnd: periodEndFromObject(object),
      plan: parseStripePlan(metadataOf(object).plan) ?? planFromStripePrice(prices, priceId),
      clearPlan: false,
    };
  }
...
function invoicePriceId(object: Record<string, unknown>): string | null {
  ...
  const first = Array.isArray(lines.data) ? asRecord(lines.data[0]) : {};
  const price = asRecord(first.price);
  return asString(price.id) ?? priceIdFromObject(object);
}
