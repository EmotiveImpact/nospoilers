  ...
  sections: [
    {
      ...
      paragraphs: [
        "Release assets and uploads exist on temporary worker disk only for the scan. They are deleted in a finally path. The database is not an archive of customer source. Packed formats include npm tarballs, zip, VSIX, Python wheels, JAR/WAR, NuGet packages, Chrome/Firefox extensions, Ruby gems, Docker/OCI image archives, and Electron asar. Encrypted zip, CRX wrappers without a ZIP payload, and encrypted image layers are inconclusive, never a passing receipt. Overlay whiteouts are not applied; lower-layer files stay inspected. Image signatures are not verified or executed. Installers (DMG/EXE/MSI/AppImage) are not scanned in this worker. Watched production websites fetch HTML plus same-origin JavaScript, CSS, and maps; those bytes are deleted after the scan. Local, private, and metadata hosts are blocked. Map custody stores encrypted Sentry/Bugsnag tokens plus debug IDs and release names from watched scans. It never downloads or keeps map source.",
      ],
    },
    {
