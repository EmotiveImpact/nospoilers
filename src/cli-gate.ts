...
export async function runGate(input:{api:string;token:string;stream:string;upload?:string;release?:string;digest:string;deployment:string;decision?:string},transport:typeof fetch=fetch){
  ...
  if(!/^[a-f0-9-]{36}$/i.test(input.stream)||!/^[a-f0-9]{64}$/.test(input.digest)||Boolean(input.upload)===Boolean(input.release))throw new Error('Supply a stream, exactly one upload or release, and the exact lowercase SHA-256 digest.');
  if(input.upload&&!/^[a-f0-9-]{36}$/i.test(input.upload)||input.release&&!/^[1-9]\d{0,14}$/.test(input.release))throw new Error('Invalid scan record identity.');
  const record=input.release?{kind:'release',id:input.release}:{kind:'upload',id:input.upload!};
  ...
  const decision=input.decision?{id:input.decision}:await request({action:'evaluate',requestKey:randomUUID(),expectedPolicyRevision:revision,record,digest:input.digest,deploymentId:input.deployment});
  if(typeof decision.id!=='string')throw new Error('Gate decision response was incomplete.');
  const result=await request({action:'consume',decisionId:decision.id,record,expectedPolicyRevision:revision,digest:input.digest,deploymentId:input.deployment});
