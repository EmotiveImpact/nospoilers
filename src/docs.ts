...
export const DOCS_SECTIONS: DocsSection[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "Check a signed receipt JSON on Scan without unpacking. The optional pack is hashed in the browser; those bytes are not uploaded. Watch lists the linked receipt status on Releases and downloads the signed JSON. Public GitHub Release download URLs and public npm tarball URLs are attached when we seal a revision. An install admin can attach another HTTPS delivery URL and verify it now: we stream-hash the bytes against the sealed digest and delete the download. Cross-host redirects are not followed, except the GitHub Release download hop onto GitHub’s asset CDN, a same-bucket S3 hop, or a same-account R2 hop (then DNS is checked again). Verify records hop hosts, a cache token, and a region when the host names one. Query strings never appear on Watch. This is not the hourly poller and not scheduled CDN verification. An authentic failed-policy or inconclusive receipt is not a clean bill of health and is not allowed to ship. The CLI is `npx nospoilers verify ./package.tgz --receipt receipt.json`. Coverage ended still allows the receipt check; attaching or verifying a remote URL needs coverage.",
