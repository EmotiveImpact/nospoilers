...
export const DOCS_SECTIONS: DocsSection[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "Check a signed receipt JSON on Scan without unpacking. The optional pack is hashed in the browser; those bytes are not uploaded. Watch lists the linked receipt status on Releases and downloads the signed JSON. An install admin can attach an HTTPS delivery URL and verify it now: we stream-hash the bytes against the sealed digest and delete the download. Cross-host redirects are not followed. Query strings never appear on Watch. This is not the hourly poller and not scheduled CDN verification. An authentic failed-policy or inconclusive receipt is not a clean bill of health and is not allowed to ship. The CLI is `npx nospoilers verify ./package.tgz --receipt receipt.json`. Coverage ended still allows the receipt check; attaching or verifying a remote URL needs coverage.",
