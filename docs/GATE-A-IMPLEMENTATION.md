...
The Railway project and dedicated worker service are provisioned. The fresh Vercel project, project-scoped credential and immutable VCR scanner digest are configured, and the Railway worker preflight has completed a clean scan in a Vercel Sandbox microVM. Railpack intentionally does not attempt Docker-in-Docker. A remote `DOCKER_HOST` remains rejected because the daemon would resolve the read-only bind source outside the worker's staged-file filesystem.

The owner then selected a fresh Neon database created through the Vercel Marketplace for the shared hosted data plane. Vercel receives the pooled `NEON_DATABASE_URL`; the persistent Railway worker receives the same database's unpooled URL as `DATABASE_URL`. The application now prefers the scoped Neon pool when present, while its existing migration and LISTEN paths derive or use a direct Neon connection. The database secret was rotated after the handoff and the replacement was installed on Railway. Redeploy and live `/api/ready` proof remain required before the database cutover is accepted. The earlier Railway PostgreSQL service must not be treated as shared production storage after that cutover.
...
1. Finish the Neon web/worker credential cutover and redeploy both runtimes. Then run live hostile archive, denied network/filesystem, timeout/resource and cleanup-after-worker-death acceptance. Repository tests, one clean Sandbox scan and successful Sandbox creation alone are not complete hostile-input runtime evidence.
...
The repository-side adapter, fresh Vercel project, scoped token, digest-pinned image and a clean Railway-to-Sandbox preflight are implemented. The Neon cutover, full hostile-input/cleanup matrix and hosted customer journey remain incomplete at this checkpoint. Enterprise SSO/SCIM, independent organization ownership, compliance evidence and the physical web/app monorepo split remain later work; this implementation does not imply they exist.
