...
| App-generated setup PR | Partial: reviewable PR, never merged; Contents write commits the vendored Action (proven on `EmotiveImpact/nospoilers-throwaway` `nospoilers/setup`); `.github/workflows/nospoilers.yml` stays copy-paste (Workflows write is not requested; 404 on that path); 409 names the committed branch/paths until Pull requests write. `npm run phase1:setup-pr` opens that PR on this throwaway when the App requested and the install accepted Pull requests write; otherwise it prints the App permissions URL and Accept URL. Do not grant Administration or Workflows write. Do not invent `-vis` | NoSpoilers |
