...
| Do-not-contact | Built: owner/repo, package name, or contact; blocks case create unless `researchOnly`; always blocks `contacted` | Internal NoSpoilers |
...
| Vendor replies and attachments | Built: owner-only append-only vendor replies; encrypted expiring attachments (text/PDF/PNG/JPEG, 64 KiB, archives rejected); bytes never appear on reports; customer 401; nothing mailed. Live on prettier: reply + `vendor-note.txt`; `.tgz` and zip magic 400 | Internal NoSpoilers |
