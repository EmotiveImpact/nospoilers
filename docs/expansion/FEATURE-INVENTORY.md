...
| Real throwaway-repository proof | Built: created-public + cheap `.env`/`.map` push + private → public (jobs 52/53 → alerts 41/42) + `release.published` → `release_scan` done → Watch “Spoilers in … phase1-fixture” + `failed-policy` receipt (MAP-001/002/003) + App-generated setup PR https://github.com/EmotiveImpact/nospoilers-throwaway/pull/1 (`nospoilers-dev[bot]`, `nospoilers/setup` → `main`, open, never merged; only `.github/actions/nospoilers/action.yml`) on `EmotiveImpact/nospoilers-throwaway`. Contents write and Pull requests write on this install; Workflows write not requested; Administration not granted. Transfer / collaborator / fork still need a real GitHub action | NoSpoilers |
| Installation-scoped billing account | Built: 5-day trial per GitHub install | NoSpoilers |
...
| Card-on-file 5-day trial | Built: Checkout always collects a payment method; remaining trial days become `trial_period_days` | NoSpoilers |
...
| Pre-publish CI gate | Partial: this repo’s GitHub Actions rebuilds fixtures, fail-closes every dirty pack, treats `inconclusive.*` as CLI exit 2 (not a passing receipt), and passes every clean pack plus workspace.tgz; dogfoods `uses: ./` on clean.tgz (pass) and sourcemap.tgz (fail closed); generated customer workflow vendors `.github/actions/nospoilers` and POSTs each existing `package.tgz` and `dist/` pack (cap 8) to hosted `/api/v1/scan`; fails closed if none; source pushes are not unpacked; Watch shows the current HTTPS origin for `NOSPOILERS_API_URL` when GitHub-hosted runners can reach it; Contents write can commit the Action; workflow YAML stays copy-paste; live App-generated setup PR is https://github.com/EmotiveImpact/nospoilers-throwaway/pull/1 (never merge); a Watch token is still needed to run hosted `/api/v1/scan` from that Action | NoSpoilers |
| App-generated setup PR | Built: reviewable PR, never merged. Live 3 Sep 2026: `npm run phase1:setup-pr` opened https://github.com/EmotiveImpact/nospoilers-throwaway/pull/1 as `nospoilers-dev[bot]` from `nospoilers/setup` onto `main` after install `158159401` accepted Pull requests write. One file: `.github/actions/nospoilers/action.yml`. `.github/workflows/nospoilers.yml` stays copy-paste (Workflows write is not requested). Do not merge. Do not grant Administration or Workflows write. Do not invent `-vis` | NoSpoilers |
...
| npm/GitHub attestation adapters | Built: trial/Team admin refreshes GitHub repo attestations (install token, 404/403 → missing) and public `registry.npmjs.org` attestation documents for a sealed digest; stores presence, subject digest, predicate type, builder id, issuer host; never stores signature/bundle bytes; never verifies Sigstore; first refresh is baseline (missing allowed, no alert); later present→missing/changed/mismatch writes `release_attestation_lost` / `_changed` / `_mismatch`; typed coordinate; members GET; Solo 403; unpaid 402; other tenant 404; hosted `api:` coordinates 400; private registry stores `missing` and is not fetched; append-only `release_attestations`. Live Neon on install `158159401`: `061` applied; unauth GET/POST 401; throwaway `phase1-fixture` revision 6 (`c74219d2…`) POST 201 github `missing` baseline, no alert; second refresh leftover row 1 stays (2 rows); UPDATE rejected; `watched_packages` 0; open jobs 0. Not a malware verdict. SBOM / Sigstore verify / scheduled CDN stay Planned | NoSpoilers |
...
| Customer-managed signing policies | Built: trial/Team admin saves one install policy (`require` GitHub and/or npm present, optional builder prefix, optional expiration); typed `signing-policy` / `clear-signing-policy`; members GET; Solo 403; unpaid 402; other tenant sees `policy: null` / 403 on write; approve-to-ship is 409 until required attestation facts match; expired policies do not block; clear deletes the row; never verifies Sigstore; never stores signatures. Live Neon: `062` applied; unauth PUT 401; GET `{ policy: null }`; PUT require-github 200 on install `158159401`; GET returned the policy; DELETE leftover 0; `watched_packages` 0. Not a malware verdict. SBOM / Sigstore verify / scheduled CDN stay Planned | NoSpoilers Team |
...
| Human lookalike allowlist | Built: trial/Team admin allowlists a registered lookalike with a reason (`POST /api/packages/:id/candidates/:candidateId/allowlist`) and may revoke it; typed candidate-name confirm; members GET candidates; Solo 403; unpaid 402; other tenant 404; allowlisted names skip further lookalike alerts and drop out of the risk score; audit stores the public package and candidate names only; never downloads tarballs; never a malware verdict. Unit-tested. Live Neon on install `158159401`: watch list empty, so no live allowlist row. No EmotiveImpact-owned npm pack to protect. Other registries stay out | NoSpoilers Team |
...
| Production website JS/CSS/assets | Built: HTTPS origin, same-origin JS/CSS/maps plus bounded probes for exposed files, credentials, and linked internal paths, SSRF blocked, never executed; public maps with `sourcesContent` are reconstructed in memory under scanner limits; scans are heavy to bound concurrent memory and consume a daily unpack slot only when changed bytes are scanned | NoSpoilers |
| Customer website ownership verification | Built: install admin proves hostname control by generated DNS TXT or exact HTTPS well-known file; challenge remains visible only to authenticated install users; method and verified timestamp stored; resetting proof revokes deploy token | NoSpoilers |
| Provider-neutral deployment trigger | Built: verified install admin mints one-time `nsd_` token stored as SHA-256 hash; `POST /api/v1/deploy` accepts provider + deployment ID, deduplicates idempotently, enqueues existing `web_origin_scan`, applies coverage/fair-use, and never trusts a caller-supplied target URL | NoSpoilers |
| Native Vercel/Netlify/Cloudflare deployment integrations | Planned convenience adapters over the provider-neutral deployment trigger; no separate scanner | NoSpoilers |
...
| Electron DMG | Deferred isolated worker: normal worker classifies and skips with a Watch alert; does not download or unpack. Live Echo `v0.1.7` job 46 / alert 35 | NoSpoilers |
| Electron EXE/NSIS | Deferred isolated worker: normal worker classifies and skips with a Watch alert; does not download or unpack. Live Echo `v0.1.7` job 46 / alert 35 | NoSpoilers |
| Electron AppImage | Deferred isolated worker: normal worker classifies and skips with a Watch alert; does not download or unpack. Live Echo `v0.1.7` Hearback AppImage skipped on job 46 / alert 35 | NoSpoilers |
...
| Watch desk 2B monolith | Built: authenticated `/watch` uses the 2B sidebar and extracted screens. Legacy `?as=trial|ended` parameters are ignored; coverage comes from the signed-in installation. Ember stays homepage-only. Live APIs; no preview tenant. Chrome comps `19`–`21`; shot C is Linear view tokens on `.watch-stage` only | NoSpoilers |
