...
| Contact history and disclosure deadlines | Built: stored security contact / https policy URL (never fetched); vendor domains persist on the organization when those fields are saved; simulated acknowledgement; internal `deadlineMissed` flag only; no mail | Internal NoSpoilers |
