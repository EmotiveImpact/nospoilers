...
| Real throwaway-repository proof | Built: created-public + cheap `.env`/`.map` push + private → public (jobs 52/53 → alerts 41/42) + `release.published` → `release_scan` done → Watch “Spoilers in … phase1-fixture” + `failed-policy` receipt (MAP-001/002/003) on `EmotiveImpact/nospoilers-throwaway`. Contents write on this install only; Workflows write not requested; Administration not granted. Transfer / collaborator / fork still need a real GitHub action | NoSpoilers |
...
| App-generated setup PR | Partial: reviewable PR, never merged; Contents write commits the vendored Action (proven on `EmotiveImpact/nospoilers-throwaway` `nospoilers/setup`); `.github/workflows/nospoilers.yml` stays copy-paste (Workflows write is not requested; 404 on that path); 409 names the committed branch/paths until Pull requests write. `npm run phase1:setup-pr` opens that PR on this throwaway when the App requested and the install accepted Pull requests write; otherwise it prints the App permissions URL and `/permissions/update` review URL (the Configure page has no Accept until that Save). Do not grant Administration or Workflows write. Do not invent `-vis` | NoSpoilers |
