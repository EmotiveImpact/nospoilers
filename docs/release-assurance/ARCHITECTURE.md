...
## Hosted worker and notification boundary (21 September 2026)

The hosted worker entrypoint is `npm run worker:hosted`. Its preflight requires the worker role, production mode, external Postgres, the hosted HTTPS origin, complete GitHub App/OAuth configuration, separate strong session/webhook/receipt secrets, an explicitly selected Slack or Resend delivery path and one reviewed isolated executor. It then performs a real clean scan through that executor. A configured flag or visible image is insufficient. Process mode and the local-review bypass remain development-only.

The selected hosted boundary is a persistent Railway queue worker dispatching each untrusted input to a fresh Vercel Sandbox microVM. `NOSPOILERS_SCANNER_MODE=vercel-sandbox` requires a project-scoped Vercel token/team/project identity and a VCR repository or fully qualified VCR reference pinned by `sha256` digest. The sandbox is nonpersistent, uses one vCPU, starts the custom image as uid65532 for staging, runs the parser as separate uid65533, receives only bounded read-only input, has deny-all network policy, writes a bounded report to `/tmp`, and is stopped after every success or failure. The image permits uid65532 to transition only to the lower-privilege parser identity; it has no root sudo grant. Before parsing, the worker proves uid65533 cannot write the sealed staging tree. Control-plane credentials remain in Railway and are not forwarded into the VM. Layered parser, command, session and cleanup deadlines fail closed. The adapter rejects symlinks, special files, oversized input/entry counts, mutable tags, other registries, oversized reports and unexpected execution identities.

The existing local-container executor remains supported where the worker owns a local daemon and mount namespace. `DOCKER_HOST` stays rejected because a remote daemon resolves bind paths outside the worker filesystem. Vercel Sandbox has a different provider control surface from the local Docker profile: the Node heap and sandbox vCPU bound are explicit, while Docker-specific PID/capability/root-filesystem flags do not have one-for-one API equivalents. Gate A therefore remains open until the fresh Vercel project contains the reviewed digest and live acceptance proves boot, denied egress, hostile input limits, timeout/cleanup and saved-result behavior. Repository tests are implementation evidence, not provider or deployment acceptance.

Migration `122_connected_workspace_notification_outbox` changes only future alert publication. An `AFTER INSERT` trigger queues each new workspace alert, including an alert tied to a connected GitHub installation, for independently owned email/Slack destinations in that same active workspace. The destination configuration version is captured, the worker rechecks the current workspace/destination/entitlement under locks, and delivery records remain workspace-scoped. The delivery body is a fixed sign-in prompt; it contains no finding body, source content or evidence URL. Existing installation-scoped notifications retain exact installation ownership. The migration deliberately performs no alert scan, backfill or historical send, and GitHub delivery retries remain idempotent.
...
Core assistance remains deterministic rule-based guidance. It cannot infer a responsible PR from a timestamp, use discovered credentials, execute changes, alter policy, approve exceptions, merge or send disclosures. Scoped MCP tools and human-reviewed notes are implemented (AGENT-TOOLS.md). Optional aggregate explanations now have explicit consent, budget reservations, cancellation and human review through an injected adapter contract, described below. A live provider adapter, provider-specific evaluation and finding-level root-cause assistance remain separate unfinished work; production has no configured model provider.
# Optional explanation adapter boundary — 9 September

`withReleaseIntelligence` accepts an optional explicit `ExplanationProvider` injection; normal production composition supplies none. GET/POST `streams/:id/explanations` use existing session/source/CSRF/request-size controls. `ra_010_release_explanations` stores immutable snapshot-bound drafts/reviews and a separate workspace UTC-day call/reserved-cost ledger. The adapter receives only aggregate states/counts and fixed instructions, has no tool/write authority and cannot issue receipts. Config-bound consent, current-access rechecks, 20-second request timeout and durable cancellation prevent stale output acceptance. No provider SDK, key or paid service is enabled. See EXPLANATIONS.md for activation limits and BUILD-LOG for verification.
