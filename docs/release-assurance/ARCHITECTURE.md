...
The hosted worker entrypoint is `npm run worker:hosted`. Its preflight requires the worker role, production mode, external Postgres, the hosted HTTPS origin, complete GitHub App/OAuth configuration, separate strong session/webhook/receipt secrets, an explicitly selected Slack or Resend delivery path, container scanner mode and an immutable scanner image reference. It then performs a real clean scan through the isolated executor. That probe exercises the same worker-local staging and read-only bind mount used for customer artifacts. A configured flag or visible image is insufficient. `DOCKER_HOST` must remain unset because a remote daemon resolves bind paths outside the worker filesystem. Process mode and the local-review bypass remain development-only.

The checked-in Railpack service does not supply that executor or scanner image. It therefore fails closed today; this is repository readiness evidence, not Railway deployment acceptance. The hosted service must provide a local container-compatible executor with the staged path visible in the same mount namespace, or the scanner needs a separately reviewed sandbox adapter with equivalent network, filesystem, UID, capability, memory, CPU, PID, timeout, output and cleanup controls. The Vercel branch deployment guard for `codex/release-assurance-spine-v1` remains disabled. No provider, deployment or paid resource is activated by this contract.
