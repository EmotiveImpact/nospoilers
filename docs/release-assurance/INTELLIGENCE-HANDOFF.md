...
21 September hosted-worker continuation: `main` contains the release-candidate merge and Railway Railpack fix. The current increment adds a reviewed `vercel-sandbox` scanner mode so the persistent Railway worker can use a fresh, deny-all, nonpersistent Vercel microVM for each artifact instead of requiring Docker-in-Docker. Do not reuse the previous Vercel account: the owner selected a fresh account/project. Remaining operator work is to import the repository into that project, build `Dockerfile.scanner` into its VCR, record the immutable digest, create a project-scoped token, configure `VERCEL_TOKEN`/`VERCEL_TEAM_ID`/`VERCEL_PROJECT_ID` and switch Railway to `NOSPOILERS_SCANNER_MODE=vercel-sandbox`. Then run the Gate A live hostile-input, denied-egress, timeout/cleanup and hosted sign-in→scan→saved-result checklist. Until that passes, the Railway worker should remain fail-closed and production readiness must not be claimed. Stripe remains owner-deferred. Final source verification for this increment passed 1,654/1,654 tests across270 files, typecheck, frontend/API builds, lint with existing warnings and diff checks.
...
Latest local integration checkpoint: New Scan has a connected repository picker and exact-job status/retry; Coverage-to-Alerts retains server-enforced source filtering; operational missing-release warnings no longer imply exposure. Real signed-in Codex browser at 4347 is working with the separate local GitHub installation. Do not restore synthetic customer review. See NEXT-SESSION-HANDOFF.md for exact current verification and the now-completed first-real-artifact proof. Full restricted regression plus permission-corrected affected files, frontend/API builds and lint are recorded in BUILD-LOG.

History refresh now removes old child-tool actions until the requested view is confirmed, with a skeleton and cancelled child lifetimes. Actual browser reproduction/recheck covers the old agent-access action; gate binding stays on the current page release. Final regression1,365tests/234files passes, as do frontend/API builds and lint with existing warnings. Browser comparison/conflict/retry, both detail hard reloads and narrow mobile reduced-motion keyboard evidence are in BUILD-LOG; these do not imply full assistive-technology certification.

Automatic capture recovery increment: selected stream/record identity resets requests and consent, failures hide stale actions, unknown settings are Checking/Unavailable rather than Off, ordinary reads use skeletons, and failed-save focus recovery respects focus moved elsewhere. Ten focused rendered checks and frontend/API builds pass; final cumulative regression passes1,363tests/234files. Isolated actual upload UI reference revoke and exclusion/restore passed; see BUILD-LOG for screenshot/mobile scope and remaining browser journeys. No customer source rule or live provider enabled.

Latest explanation increment: selected recorded history now mounts an optional explanation disclosure backed by real session-scoped routes. ra_010 adds private snapshot-linked drafts and workspace-wide daily budget reservations. Operator-injected adapter contract is disabled by default; explicit config-bound consent, cancellation, idempotency and human review do not alter deterministic evidence. Read EXPLANATIONS.md and latest BUILD-LOG. Older 'provider contracts unbuilt' notes are historical; concrete provider integration/activation and provider-attested provenance remain separate unfinished work.

Owner update: customer-facing synthetic dev review is removed, including its launcher, friendly sign-in, banner and automatic stream seeding. Keep internal QA fixtures for testing only; do not launch a fake workspace as the owner's application. All identified local servers were stopped. Real GitHub setup/live verification is deferred to production at the owner's request, not accepted as complete. Use the four-agent continuation file without repeated remote polling.

Current Gate C batch: production website eligibility is explicit and fail-closed in the selector; mutation errors in Gate, Remediation and Agent tools recover keyboard focus without stealing focus from another control. History records have distinct accessible names and selection announcements. Ordinary agent/remediation reads use existing skeletons. See latest BUILD-LOG for actual verification, not historical totals.
