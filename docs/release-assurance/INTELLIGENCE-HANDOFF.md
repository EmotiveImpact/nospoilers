...
21 September hosted-worker continuation: `main` contains the release-candidate merge and Railway Railpack fix. The fresh `no-spoilers/nospoilers` project now has a VCR scanner repository; Railway production has the project-scoped token, team/project IDs, `vercel-sandbox` mode and immutable digest. The image uses uid65532 for staging and uid65533 for parsing, with the SDK-required sudo transition restricted to the parser identity and no root grant. A live clean isolated scan and the full production worker preflight pass. Remaining Gate A work is the hostile-input, denied-egress, timeout/interruption cleanup and hosted sign-in→repository→scan→saved-result checklist. Until those pass, production readiness must not be claimed. Stripe remains owner-deferred. Final source verification passed1,654/1,654 tests across270 files, focused adapter/readiness/Gate A tests passed28/28, and typecheck, frontend/API builds, lint with existing warnings and diff check passed.
