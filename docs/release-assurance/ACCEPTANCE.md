...
21 September hosted isolation adapter: the repository now supports a Railway worker dispatching untrusted artifacts to fresh Vercel Sandbox microVMs through `@vercel/sandbox` 3.3.0. Production preflight accepts only the existing worker-local container boundary or the new project-scoped Sandbox boundary. The latter requires a digest-pinned VCR image and explicit Vercel token/team/project scope, uses deny-all networking, no forwarded control-plane credentials, uid 65532, bounded read-only staging/report sizes, layered deadlines and mandatory stop. Final unrestricted regression passed **1,654/1,654 tests across 270 files**; focused adapter/readiness/Gate A tests passed 28/28, and typecheck, frontend/API builds, lint with the existing warning set and diff checks passed. This is repository implementation only. The owner selected a fresh Vercel account; its project/token/image digest are not configured, the Railway worker remains fail-closed, and live hostile-input/egress/cleanup plus hosted sign-in→scan→saved-result acceptance remain required before Gate A or production readiness can be declared.
