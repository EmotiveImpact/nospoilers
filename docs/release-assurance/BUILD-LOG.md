## 21 September 2026 — live VCR image and Railway worker preflight

Provisioned the fresh `no-spoilers/nospoilers` Vercel Container Registry repository and built `Dockerfile.scanner` remotely inside an ephemeral Vercel Sandbox, without installing Docker on the operator host. Railway production now holds the project-scoped Vercel access token, team/project IDs, `vercel-sandbox` mode and the immutable scanner image `vcr.vercel.com/no-spoilers/nospoilers/nospoilers-scanner@sha256:1960a805b85e9b9c2ba148bf5d075b2733aa2b010b3ee909e63201a9ad555c6c`. Registry pushes use a short-lived project OIDC token minted from the Railway-held access token; neither credential entered the image, repository or logs.

Live probing exposed and corrected a mismatch between the Sandbox multi-user SDK and the first image. The image now contains a fixed uid65532 staging user and a separate uid65533 parser user. Its sudoers rule permits the staging user to transition only to `nospoilers-parser`, never root. Uploaded files remain0444 and directories0555 under the staging identity; the worker proves the parser identity is distinct/non-root and proves it cannot create a file in the sealed input tree before executing the parser. The parser receives no control-plane credentials and the session still uses deny-all network, nonpersistent storage, bounded input/report sizes and mandatory stop.

Focused adapter/readiness/Gate A tests passed28/28, the live clean scan returned `ok:true`, and the complete production preflight returned `{"ok":true,"role":"worker","database":"postgres","scanner":"vercel-sandbox-microvm","notification":"slack"}`. Final source verification passed **1,654/1,654 tests across270 files** in297.97s plus typecheck, frontend build, API build, lint with the existing warning set and diff check. This establishes the configured worker startup contract and one clean isolated execution. Gate A remains open for live hostile-input, denied-egress, timeout/interruption cleanup and the hosted GitHub sign-in→repository→scan→saved-result journey. Stripe remains owner-deferred.
...
## 21 September 2026 — fresh Neon database staged for Vercel and Railway

The owner selected a new production database rather than exposing the Railway PostgreSQL service over a public TCP proxy. Created Vercel Marketplace Neon resource `nospoilers-production` in `iad1` on the free plan and connected it to the fresh `nospoilers` project for Production and Preview. Preview branching is enabled; production branching is disabled. Neon Auth remains off because NoSpoilers keeps its GitHub OAuth/session and workspace-authority model. Vercel injected sensitive variables with the `NEON_` prefix, avoiding replacement of the existing `DATABASE_URL` before a controlled cutover.

`loadConfig` now prefers `NEON_DATABASE_URL` when present and falls back to `DATABASE_URL`, so the serverless web runtime uses the Marketplace pool while the persistent Railway worker can use the same database's unpooled connection as `DATABASE_URL`. Existing migration logic converts a pooled Neon host to a direct host and takes the PostgreSQL advisory lock; the existing LISTEN path also requires a direct connection. `.env.example` records the division without adding credentials. A regression proves Neon precedence and worker fallback.

Validation on the final source: focused hosted/production-runtime/job-wake tests passed **75/75 across 3 files**; `npm run typecheck`, frontend build and API build passed; lint exited 0 with the existing 33 warnings; `git diff --check` passed. The complete regression first produced the known restricted-environment CLI/localhost failures, then passed **1,655/1,655 tests across 270 files** in303.69s with normal subprocess/localhost access. Two non-failing tar locale messages remained.

The first controlled handoff exposed the database URL in browser accessibility output. The Neon resource secret was therefore rotated immediately with the recorded reason `Rotate after credential exposure during production setup`; old values expired, and only the replacement unpooled URL was installed as Railway `DATABASE_URL`. A masked Railway check confirms a direct `.neon.tech` host and `neondb` database. The cutover is not yet accepted: neither runtime has been redeployed on this change, and shared `/api/ready`, worker preflight and hosted sign-in→scan→saved-result evidence have not been rerun. The earlier Railway PostgreSQL service remains intact pending a verified cutover and must not be deleted as part of this increment.
