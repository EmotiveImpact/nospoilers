...
The earlier gallery mixed layout studies, state studies, and workflow walkthroughs. `2A`–`2D`
fix that. They are four complete versions of the same current-main capability set, all copied from
the same full click-through and all carrying the same five state presets and incident lifecycle.
Only the default surface and information architecture differ.
