...
1. **Packed bytes** — the npm tarball, zip, or Electron `app.asar` about to go to customers. Same class of leak as Claude Code’s `cli.js.map` on npm and source maps inside a public desktop installer.
...
| Extra | Email | Slack + Jira + routing + 90-day timeline + roles + audit export + identity signals |
...
- Scanner kernel: src/scanner/ (dir, tgz, zip, asar; MAP-*, SEC-*, GIT-*, SRC-*, SIZE-*)
...
5. When a release is published, download the release asset(s) that are .tgz/.tar.gz/.zip/.asar (skip huge/irrelevant files with a size cap), run the EXISTING scanner, show findings. Never keep the unpacked bytes after the scan — store finding rows only.
