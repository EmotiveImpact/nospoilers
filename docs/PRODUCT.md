...
1. **Packed bytes** — the npm tarball, zip, VSIX, wheel, JAR, gem, Docker/OCI image, or Electron `app.asar` about to go to customers. Same class of leak as Claude Code’s `cli.js.map` on npm and source maps inside a public desktop installer.
...
| Piece | Where |
| --- | --- |
| Scanner kernel | `src/scanner/` — dir, `.tgz`/`.tar.gz`, `.zip`, `.vsix`, `.crx`, `.xpi`, `.whl`, `.jar`/`.war`, `.nupkg`, `.gem`, Docker/OCI image tar, Electron `.asar`; npm/pnpm/Yarn/Bun workspace listing |
...
- Scanner kernel: src/scanner/ (dir, tgz, zip, vsix, crx, xpi, whl, jar, nupkg, gem, docker/oci, asar; MAP-*, SEC-*, GIT-*, SRC-*, SIZE-*)
...
5. When a release is published, download the release asset(s) that are packed artifacts (.tgz/.tar.gz/.tar/.zip/.asar/.vsix/.crx/.xpi/.whl/.jar/.war/.nupkg/.gem) (skip huge/irrelevant files with a size cap), run the EXISTING scanner, show findings. Never keep the unpacked bytes after the scan — store finding rows only.
