...
add('API pending scan does not produce a passport or invented result',async()=>{const h=harness({pending:true});const r=await handleAssuranceRequest(h.req(`/api/assurance/uploads/${UUID}`),h.deps);assert.equal(r.status,202);const b=await r.json();assert.equal(b.pending,true);assert.equal(b.view,undefined);});
add('API invalid signature cannot become a ready assessment',async()=>{const h=harness();h.deps.verify=()=>({ok:false});const r=await handleAssuranceRequest(h.req('/api/assurance/releases/2'),h.deps);assert.equal(r.status,200);assert.equal((await r.json()).view.assessment.beforeDeploy,'unknown');});
...
add('API distinguishes unreadable history from first-release history',async()=>{const h=harness();const read=h.deps.read;h.deps.read=request=>new URL(request.url).pathname==='/api/releases'?response({},503):read(request);const r=await handleAssuranceRequest(h.req('/api/assurance/releases/2'),h.deps);assert.match((await r.json()).view.comparison.reason,/could not be read/);});
