...
export function HostedBriefFixture({mode,canPublish=false,published=false,onConfirm}:{mode:string;canPublish?:boolean;published?:boolean;onConfirm?:(value:unknown)=>void}){
 const release=useMemo<ReleaseRevision>(()=>({...baseRelease,receiptStatus:mode==='brief'?'failed-policy':'passed',publicPage:published?{enabled:true,path:'/verify/example'}:null,legalHold:mode==='brief-blocked'?{active:true,actorLogin:'fixture-owner',reason:'Fixture legal hold',createdAt:baseRelease.createdAt}:null}),[mode,published]);
 ...
 const context={Button,activeInstallId:1,selectedInstallId:1,attachingReleaseId:null,canGovernReleases:false,canPublishVerify:canPublish,beginConfirm:onConfirm??(()=>{}),confirming:null,confirmForm:()=>null,deliveryError:null,deliveryUrlByRelease:{},downloadingReceiptId,formatSealedBytes:(n:number)=>`${n} bytes`,governanceReasonByRelease:{},installAdmin:false,loadJson:async()=>{throw new Error('Fixture: downloads disabled.');},locked:true,navigate,receiptError,refreshSignedIn:async()=>{},releases:[release],search,setDownloadingReceiptId,setReceiptError,verifyingLocationId:null,watchHref,watchPath} as unknown as WatchScreenContext;
