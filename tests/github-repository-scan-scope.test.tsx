...
it('aborts old progress requests and ignores a late completion after workspace switch',async()=>{
 ...
 expect(signal?.aborted).toBe(true);await screen.findByLabelText('Repository');
 await act(async()=>{finish(Response.json({jobs:[{id:9,installationId:7,kind:'scan_latest_release',status:'done'}]}));});
 expect(screen.queryByText(/Release check finished/)).toBeNull();expect((screen.getByLabelText('Repository') as HTMLSelectElement).value).toBe('');
});
