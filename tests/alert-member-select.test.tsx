...
it('uses workspace assignees and clears the previous member list on scope changes',async()=>{
 ...
 const props={alertId:1,value:'',onChange:vi.fn(),inputRef:createRef<HTMLButtonElement>()};
...
it('selects internal member IDs, not display names, without assigning on mount',async()=>{
 ...
 await waitFor(()=>expect(screen.getByRole('combobox')).toHaveProperty('disabled',false));expect(onChange).not.toHaveBeenCalled();
 await selectOption(screen.getByRole('combobox'),'Taylor');expect(onChange).toHaveBeenCalledWith('internal-1');
 expect(fetch.mock.calls[0][0]).toBe('/api/alerts/1/assignees');
 await selectOption(screen.getByRole('combobox'),'Unassigned — clear ownership');expect(onChange).toHaveBeenLastCalledWith(CLEAR_ALERT_ASSIGNMENT);
});
