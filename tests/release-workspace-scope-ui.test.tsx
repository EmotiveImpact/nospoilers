...
it.each([['?workspace=w&install=7','Recorded releases'],['?workspace=w&install=7&releaseView=connected','Connected releases'],['?workspace=w&install=7&preview=7','Connected releases'],['?workspace=w&install=7&releaseView=connected&upload=attempt','Recorded releases']])('restores the history surface from %s', (search,name)=>{
...
it('switches history with keyboard while clearing only conflicting selection context',()=>{
 ...
 const view=render(<ReleasesScreen/>);const attempts=screen.getByRole('tab',{name:'Recorded releases'});
