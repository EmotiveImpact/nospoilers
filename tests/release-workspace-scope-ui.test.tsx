...
it.each([['?workspace=w&install=7','Saved attempts'],['?workspace=w&install=7&releaseView=connected','Connected revisions'],['?workspace=w&install=7&preview=7','Connected revisions'],['?workspace=w&install=7&releaseView=connected&upload=attempt','Saved attempts']])('restores the history surface from %s', (search,name)=>{
...
it('switches history with keyboard while clearing only conflicting selection context',()=>{
 ...
 const view=render(<ReleasesScreen/>);const attempts=screen.getByRole('tab',{name:'Saved attempts'});
 attempts.focus();fireEvent.keyDown(attempts,{key:'ArrowRight'});
 expect(document.activeElement).toBe(screen.getByRole('tab',{name:'Connected revisions'}));
 ...
 fireEvent.keyDown(screen.getByRole('tab',{name:'Connected revisions'}),{key:'Home'});
...
it('keeps connected export and shows an uploaded filename while linking the exact revision',async()=>{
 ...
 fireEvent.click(screen.getByRole('button',{name:/app.tgz/}));expect(new URL(state.navigate.mock.lastCall[0],'http://localhost').searchParams.get('preview')).toBe('7');
